...
// case 3
  ...
  motion.init(argv[1], point_init, true);
  ...
  for (int i = 0; i < cur_v.size(); i++) {
    ...
    auto res = ILQR::LongitudinalOperator::calc_idm_acc(idm_params, 0.0,
                                                        cur_v[i] / 3.6, 0.0, 0);
    std::cout << " no car, " << cur_v[i] << " -> " << desired_v[i]
              << " res : " << res.first << std::endl;
  }
  ...
  for (int i = 0; i < cur_v.size(); i++) {
    ...
    auto res = ILQR::LongitudinalOperator::calc_idm_acc(
        idm_params, delta_s[i], cur_v[i] / 3.6, (cur_v[i] - obj_v[i]) / 3.6, 1);
    std::cout << " has car, " << cur_v[i]
              << " -> desired spd is: " << desired_v[i]
              << " , obj_v is: " << obj_v[i] << " , delta s is: " << delta_s[i]
              << " res : " << res.first << std::endl;
    std::cout << "-------------------------------" << std::endl;
  }
