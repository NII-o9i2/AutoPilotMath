...
public:
  bool
  check_collision(int frame_cnt, const StateVariable &state,
...
public:
  ...
  void set_init_control(const ControlVariable &control) { init_control_ = control; }
...
private:
  StateVariable step(const StateVariable &state, const ControlVariable &action,
  ...
  void search_node(std::vector<SamplePoint> &cur_path,
                   ...
                   bool &find_res, int &count, int index,
                   StateVariable cur_state, ControlVariable last_control);

  bool check_sample_point(const double &t, const StateVariable &last_state,
