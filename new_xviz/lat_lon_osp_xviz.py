...
class XvizPlotLatLonMotion(XvizPlotBase):
    ...
    def figure_plot(self):
        ...
        self.build_osp_reference_point_data_frame()
        self.build_osp_leader_car_data_frame()
        self.build_osp_ref_line_with_leader_car_data_frame()   
        self.build_osp_select_guide_lane_points_data_frame()   
        self.build_osp_guide_points_by_lane_points_data_frame() 
        self.build_osp_guide_points_theta_by_lane_points_data_frame()
        ...
        self.add_layer_to_figure(main_figure_viz, args=dict(data_key='lat_lon_motion_match_point', \
                                                                ...
                                                                line_alpha=1, line_width=6))
        self.add_layer_to_figure(main_figure_viz, args=dict(data_key='osp_ref_line_without_leader_car', \
                                                                plot_type='scatter', color='black',  \
                                                                label='osp_ref_line_without_leader_car', \
                                                                line_alpha=0.6, line_width=6))
        
        self.add_layer_to_figure(main_figure_viz, args=dict(data_key='osp_leader_car_history_point', \
                                                                plot_type='scatter', color='pink',  \
                                                                label='osp_leader_car_history_point', \
                                                                line_alpha=0.6, line_width=6))
        
        self.add_layer_to_figure(main_figure_viz, args=dict(data_key='osp_ref_line_with_leader_car', \
                                                                plot_type='scatter', color='Gold',  \
                                                                label='osp_ref_line_with_leader_car', \
                                                                line_alpha=0.6, line_width=6)) 
        self.add_layer_to_figure(main_figure_viz, args=dict(data_key='osp_select_raw_lane_points', \
                                                                plot_type='scatter', color='Brown',  \
                                                                label='osp_select_raw_lane_points', \
                                                                line_alpha=0.6, line_width=6))
        self.add_layer_to_figure(main_figure_viz, args=dict(data_key='osp_guide_lane_points', \
                                                                plot_type='scatter', color='Cyan',  \
                                                                label='osp_guide_lane_points', \
                                                                line_alpha=0.6, line_width=6))
        
        self.figs_['main_fig'] = main_figure_viz.plot()
        callback_main_fig = main_figure_viz.get_callback_list()        
        
        # osp_guide_points_theta_by_lane_points
        osp_guide_points_theta_by_lane_points_figure_viz = FigureViz('osp_guide_points_theta_by_lane_points', 'Index', 'Data/degree', 
                                         width=550 ,height = 300,y_range = [-185, 185], match_aspect = False)
        self.add_layer_to_figure(osp_guide_points_theta_by_lane_points_figure_viz, args=dict(data_key='osp_theta_by_lane_point', \
                                                                plot_type='index_line_circle', color='blue',  \
                                                                label='osp_guide_points_theta_by_lane_points'))
        self.figs_['osp_guide_points_theta_by_lane_points'] = osp_guide_points_theta_by_lane_points_figure_viz.plot()
        callback_osp_theta_by_lane_point = osp_guide_points_theta_by_lane_points_figure_viz.get_callback_list() 
        ...
        all_callback = [*callback_main_fig, *callback_nop_count, *callback_acc, 
                ...
                *callback_lead_s,*callback_osp_theta_by_lane_point
                ]
    ...
    def build_osp_reference_point_data_frame(self):
        data_frame = self.get_data_frame_at_datakey('osp_ref_point_by_ego_car_debug')
        data_frame_new = []
        for one_frame in data_frame:
            one_frame_new = {}
            one_frame_new['t'] = one_frame['t']
        ...
        self.set_data_frame_at_datakey(
            'point_channel', 'osp_ref_line_without_leader_car', data_frame_new)
        
    def build_osp_leader_car_data_frame(self):
        data_frame = self.get_data_frame_at_datakey('osp_leader_car_history_pose_debug')
        data_frame_new = []
        for one_frame in data_frame:
            one_frame_new = {}
            one_frame_new['t'] = one_frame['t']
            ...
            data_frame_new.append(one_frame_new)
        self.set_data_frame_at_datakey(
            'point_channel', 'osp_leader_car_history_point', data_frame_new)
        
    def build_osp_ref_line_with_leader_car_data_frame(self):
        data_frame = self.get_data_frame_at_datakey('osp_ref_point_by_leader_car_debug')
        data_frame_new = []
        for one_frame in data_frame:
            one_frame_new = {}
            one_frame_new['t'] = one_frame['t']
            ...
            data_frame_new.append(one_frame_new)
        self.set_data_frame_at_datakey(
            'point_channel', 'osp_ref_line_with_leader_car', data_frame_new)
        
    def build_osp_select_guide_lane_points_data_frame(self):
        data_frame = self.get_data_frame_at_datakey('osp_select_guide_lane_points')
        data_frame_new = []
        for one_frame in data_frame:
            one_frame_new = {}
            one_frame_new['t'] = one_frame['t']
            ...
            data_frame_new.append(one_frame_new)
        self.set_data_frame_at_datakey(
            'point_channel', 'osp_select_raw_lane_points', data_frame_new)
        
    def build_osp_guide_points_by_lane_points_data_frame(self):
        data_frame = self.get_data_frame_at_datakey('osp_guide_points_by_lane_points')
        data_frame_new = []
        for one_frame in data_frame:
            one_frame_new = {}
            one_frame_new['t'] = one_frame['t']
            ...
            data_frame_new.append(one_frame_new)
        self.set_data_frame_at_datakey(
            'point_channel', 'osp_guide_lane_points', data_frame_new)
        
    def build_osp_guide_points_theta_by_lane_points_data_frame(self):
        data_frame_new = []
        ...
        for one_frame in data_frame:
            ...
            x = []
            for theta in one_frame['data']:
                x.append(theta)
            one_frame_new['data'] = x
            data_frame_new.append(one_frame_new)
        self.set_data_frame_at_datakey(
            '', 'osp_theta_by_lane_point', data_frame_new)
    ...
    def add_check_group(self):
        for fig_name in ['motion_res_jerk', 'motion_res_acc','motion_res_vel','lead_s',
                     ...
                     'refline_curvature','osp_guide_points_theta_by_lane_points']:
    ...
    def show2html(self):
        ...
        left = column(self.checkbox_groups_['motion_res_jerk'],self.figs_['motion_res_jerk'],
                      ...
                      self.checkbox_groups_['osp_guide_points_theta_by_lane_points'],self.figs_['osp_guide_points_theta_by_lane_points'])
