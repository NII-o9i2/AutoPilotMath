...
class XvizPlotLatLonMotion(XvizPlotBase):
    ...
    def figure_plot(self):
        ...
        self.build_interaction_search_success_traj_point_data_frame()
        self.build_interaction_search_failed_traj_point_data_frame()
        self.build_interaction_search_failed_ego_polygon_data_frame()
        self.build_interaction_search_success_ego_polygon_data_frame()
        self.build_freespace_mgr_road_edges_points_data_frame()
        self.build_osp_mgr_task_motion_tree_points_data_frame()
        self.build_success_nn_traj_point_data_frame()
        self.build_failed_nn_traj_point_data_frame()
        ...
        self.add_layer_to_figure(main_figure_viz, args=dict(data_key='search_success_traj_points', \
                                                                plot_type='scatter', color='green',  \
                                                                label='search_success_traj_points', \
                                                                line_alpha=1.0, line_width=6))
        self.add_layer_to_figure(main_figure_viz, args=dict(data_key='search_failed_traj_points', \
                                                                plot_type='scatter', color='red',  \
                                                                label='search_failed_traj_points', \
                                                                line_alpha=1.0, line_width=6))   
        self.add_layer_to_figure(main_figure_viz, args=dict(data_key='interaction_search_failed_ego_polygon', \
                                                        plot_type='multi_polygon', color='red',  \
                                                        label='search_failed_ego_polygon', fill_alpha=0, \
                                                        line_alpha=0.4, line_width=4))
        self.add_layer_to_figure(main_figure_viz, args=dict(data_key='interaction_search_success_ego_polygon', \
                                                        plot_type='multi_polygon', color='green',  \
                                                        label='search_success_ego_polygon', fill_alpha=0, \
                                                        line_alpha=0.4, line_width=4))
        ...
        self.add_layer_to_figure(main_figure_viz, args=dict(data_key='raw_road_edges_pts', \
                                                                ...
                                                                line_alpha=1.0, line_width=6))
        self.add_layer_to_figure(main_figure_viz, args=dict(data_key='success_nn_traj_points', \
                                                        plot_type='scatter', color='green',  \
                                                        label='success_nn_traj_points', \
                                                        line_alpha=1.0, line_width=6))
        self.add_layer_to_figure(main_figure_viz, args=dict(data_key='failed_nn_traj_points', \
                                                                plot_type='scatter', color='red',  \
                                                                label='failed_nn_traj_points', \
                                                                line_alpha=1.0, line_width=6))  
    ...
    def build_interaction_search_success_traj_point_data_frame(self):
        data_frame_new = []
        data_frame = self.get_data_frame_at_datakey('interaction_search_debug')
        ...
        for one_frame in data_frame:
            ...
            x = []
            y = []
            if 'success_search_trajs' in one_frame['data']:
                success_search_trajs = one_frame['data']['success_search_trajs']
                if len(success_search_trajs) == 0:
                    pass
                else:
                    for traj in success_search_trajs:
                        for point in traj:
            ...
            one_frame_new['data'] = one_data_new
            data_frame_new.append(one_frame_new)
        self.set_data_frame_at_datakey('traj_channel', 'search_success_traj_points', data_frame_new)

    def build_interaction_search_failed_traj_point_data_frame(self):
        data_frame_new = []
        data_frame = self.get_data_frame_at_datakey('interaction_search_debug')
        ...
        for one_frame in data_frame:
            ...
            x = []
            y = []
            if 'failed_search_trajs' in one_frame['data']:
                failed_search_trajs = one_frame['data']['failed_search_trajs']
                if len(failed_search_trajs) == 0:
                    pass
                else:
                    for traj in failed_search_trajs:
                        for point in traj:
                            x.append(point['position']['x'])
                            y.append(point['position']['y'])
            one_data_new = {
            ...
            one_frame_new['data'] = one_data_new
            data_frame_new.append(one_frame_new)
        self.set_data_frame_at_datakey('traj_channel', 'search_failed_traj_points', data_frame_new)
    
    def build_success_nn_traj_point_data_frame(self):
        data_frame_new = []
        data_frame = self.get_data_frame_at_datakey('interaction_search_debug')
        ...
        for one_frame in data_frame:
            ...
            x = []
            y = []
            if 'success_nn_trajs' in one_frame['data']:
                success_nn_trajs = one_frame['data']['success_nn_trajs']
                if len(success_nn_trajs) == 0:
                    pass
                else:
                    for traj in success_nn_trajs:
                        for point in traj:
                            x.append(point['position']['x'])
                            y.append(point['position']['y'])
            one_data_new = {
            ...
            one_frame_new['data'] = one_data_new
            data_frame_new.append(one_frame_new)
        self.set_data_frame_at_datakey('traj_channel', 'success_nn_traj_points', data_frame_new)

    def build_failed_nn_traj_point_data_frame(self):
        data_frame_new = []
        data_frame = self.get_data_frame_at_datakey('interaction_search_debug')
        ...
        for one_frame in data_frame:
            ...
            x = []
            y = []
            if 'failed_nn_trajs' in one_frame['data']:
                failed_nn_trajs = one_frame['data']['failed_nn_trajs']
                if len(failed_nn_trajs) == 0:
                    pass
                else:
                    for traj in failed_nn_trajs:
                        for point in traj:
                            x.append(point['position']['x'])
                            y.append(point['position']['y'])
            one_data_new = {
            ...
            one_frame_new['data'] = one_data_new
            data_frame_new.append(one_frame_new)
        self.set_data_frame_at_datakey('traj_channel', 'failed_nn_traj_points', data_frame_new)
    ...
    def build_interaction_search_success_ego_polygon_data_frame(self):
        data_frame_new = []
        data_frame = self.get_data_frame_at_datakey('interaction_search_debug')
        ...
        for one_frame in data_frame:
                ...
                if len(success_search_trajs) == 0:
                    ...
                    for traj in success_search_trajs:
                        ...
                        ego_polygon_x, ego_polygon_y =  \
                                self.build_polygon_by_pos_and_heading(body_center_x, \
                                                                    body_center_y, \
                                                                    point['theta'])
            ...
            one_frame_new['data'] = one_data_new
            data_frame_new.append(one_frame_new)
        self.set_data_frame_at_datakey('', 'interaction_search_success_ego_polygon', data_frame_new)
    
    def build_interaction_search_failed_ego_polygon_data_frame(self):
        data_frame_new = []
        data_frame = self.get_data_frame_at_datakey('interaction_search_debug')
        ...
        for one_frame in data_frame:
            ...
            one_frame_new['data'] = one_data_new
            data_frame_new.append(one_frame_new)
        self.set_data_frame_at_datakey('', 'interaction_search_failed_ego_polygon', data_frame_new)
    ...
    def add_check_group(self):
        for fig_name in ['motion_res_jerk', 'motion_res_acc','motion_res_vel','lead_s','osp_rp_omega',
